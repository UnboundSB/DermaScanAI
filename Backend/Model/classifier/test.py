...
def get_dynamic_test_loader():
    print("--- SCANNING DATASET FOR MINIMUM CLASS COUNT ---")
    ...
    print(f"Locking test set to exactly {min_samples} images per class.")
...
def evaluate_model(model_path, model, test_loader, classes):
    ...
    with torch.no_grad():
        for inputs, labels in tqdm(test_loader, desc="Running Inference", leave=False):
...
def generate_comparison_plots(df_metrics, df_acc):
    ...
    sns.barplot(data=df_acc, x="Model", y="Overall Accuracy", palette="Blues_d")
    plt.title("Overall Accuracy Comparison")
    plt.ylim(0.5, 1.0)
    plt.ylabel("Accuracy")
    plt.tight_layout()
    plt.savefig(os.path.join(SCRIPT_DIR, "compare_0_overall_accuracy.png"))
    plt.close()
    ...
    for i, metric in enumerate(metrics_to_plot, 1):
        ...
        sns.barplot(data=df_metrics, x="Class", y=metric, hue="Model", palette="magma")
        plt.title(f"Model Comparison: {metric} by Symptom Class", fontsize=16)
        plt.ylim(0.5, 1.0)
        plt.ylabel(metric)
        plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left')
        plt.tight_layout()
        plt.savefig(os.path.join(SCRIPT_DIR, f"compare_{i}_{metric.lower()}.png"))
        plt.close()
...
def main():
    ...
    print("\nLoading Base EfficientNet-B0 Architecture...")
    ...
    if not all_class_metrics:
        print("\n[!] No models were successfully evaluated. Check your file paths.")
        return
    ...
    print(f"\nAll individual confusion matrices and comparative bar charts have been saved to: {SCRIPT_DIR}")
